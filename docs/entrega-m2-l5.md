# Entrega — Módulo 2, Lição 5 (Mundo Real)
...
|---|---|---|
| 0:00–0:35 | contexto de negócio | o problema dos 2–4 dias perdidos e o campo `resultado` |
| 0:35–1:30 | demonstração do fluxo | `/projeto` → `/hipotese`, baseline obrigatório |
...
Arquivo: `docs/handoff-sprint.md`

**(a) Três decisões técnicas com justificativa**

1. `resultado` na modelagem desde o primeiro dia — campo que sustenta a proposta
   de valor entra na primeira migração, não na segunda.
2. IA cortada da v1 — sem registro acumulado não há o que sugerir; IA sobre base
   vazia é demonstração, não valor.
3. Falha de API avisa o usuário **e** registra no log — as duas coisas, porque não
   há motivo para escolher entre usuário informado e erro diagnosticável.

**(b) Bloqueios com owner e prazo**

| # | Bloqueio | Owner | Prazo |
|---|---|---|---|
| 1 | escopo da v1 (inclui busca por similaridade?) | produto | 1 semana |
| 2 | migrar `api/` para tRPC + Drizzle | lead técnico | 2 semanas |
| 3 | CI ausente | Joel | 1 semana |

Com a articulação de qual é o mais crítico: o 1, porque para o progresso —
...
**PR #3:** https://github.com/Joel-Cistrahenn/paris-group-copilot/pull/3

Aberto via `gh pr create --title ... --body ...` (título e corpo direto do
terminal, sem editor interativo), seguindo o template do repositório:

- **O que mudou** — handoff, roteiro de demo e índice no README.
- **Por que mudou** — investidores fazem revisão sem daily; decisão em Slack some.
- **Como testar localmente** — bloco de comandos incluindo o caminho de falha
  (derrubar a API e conferir aviso na tela + causa no log).
- **Rastreabilidade** — link do handoff, estado do vídeo, tarefas do TaskNotes e
  PRs anteriores.

### Histórico de PRs do repositório

```
PR #1  chore(docs): AGENTS.md com regras canônicas          MERGED
PR #2  feat(setup): ambiente de desenvolvimento local        MERGED
PR #3  docs(sprint): atualização assíncrona para stakeholders OPEN
```

Nenhum commit foi para a `main` sem PR desde que `workflow-policy.sh set-mode pr`
foi ativado.
